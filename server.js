...
function getUser() {
  ...
  // Never fall back to the first account: that would expose the owner's profile
  // to anyone opening the public URL without valid Telegram initData.
  return null;
}

function telegramAuthRequired(pathname) {
  return pathname.startsWith("/api/") && !pathname.startsWith("/api/cryptobot/webhook/");
}
...
async function routeRequest(req, res) {
  ...
  if (telegramAuthRequired(parsed.pathname) && !telegramAuth.valid) {
    await sendJson(res, 401, {
      error: "Открой BOR внутри Telegram Mini App: профиль не может быть определён без Telegram ID",
      code: "TELEGRAM_AUTH_REQUIRED",
      reason: telegramAuth.reason
    });
    return;
  }
