...
const ROULETTE_SEGMENTS = [
  { label: "x0", multiplier: 0, color: "#4a5560" },
  { label: "x2", multiplier: 2, color: "#e2ab48" },
  { label: "x3", multiplier: 3, color: "#3c86ff" },
  { label: "x5", multiplier: 5, color: "#24b36e" },
  { label: "x0", multiplier: 0, color: "#6a3c44" },
  { label: "x2", multiplier: 2, color: "#f1c35c" },
  { label: "x3", multiplier: 3, color: "#5f96ff" },
  { label: "x10", multiplier: 10, color: "#ca607f" }
];
...
const APP_MODE = envStr("BOR_APP_MODE", "casino mini app");
...
function makeGames() {
  return [
    { id: "mines", name: "Mines", icon: "💣", note: "доступно", theme: "mines", live: true },
    { id: "crash", name: "Crash", icon: "🚀", note: "в разработке", theme: "locked", live: false },
    { id: "slots", name: "Slots", icon: "🎰", note: "в разработке", theme: "locked", live: false },
    { id: "dice", name: "Dice", icon: "🎲", note: "в разработке", theme: "locked", live: false }
  ];
}
...
function sanitizeUser(user) {
  return {
    ...
    volume: 0,
    is_admin: 1,
    vip_level: typeof user?.vip_level === "string" && user.vip_level.trim() ? user.vip_level.trim() : "Silver"
  };
}
...
async function handleCreateInvoice(body, res) {
  ...
  if (!Number.isFinite(amount) || amount < 1) {
    await sendJson(res, 400, { error: "Минимальное пополнение 1 USDT" });
    return;
  }
  ...
  await sendJson(res, 200, {
    ...
    invoiceUrl: invoice.mini_app_invoice_url || invoice.web_app_invoice_url || invoice.bot_invoice_url || "",
...
async function handleRouletteSpin(body, res) {
  ...
  if (!Number.isFinite(bet) || bet < 1) {
    await sendJson(res, 400, { error: "Минимальная ставка 1 USDT" });
    return;
  }
  if (!spendBalance(bet)) {
  ...
  const user = getUser();
  const resultIndex = Math.floor(Math.random() * ROULETTE_SEGMENTS.length);
  const segment = ROULETTE_SEGMENTS[resultIndex];
  const payout = Number((bet * segment.multiplier).toFixed(2));
  const net = Number((payout - bet).toFixed(2));
  user.games_played += 1;
  user.volume = Number((user.volume + bet).toFixed(2));
  if (payout > 0) {
    awardBalance(payout);
    user.wins += 1;
  }
  ...
  state.lastRoulette = {
    ...
    payout,
    net,
    rotation: 1800 + Math.floor(Math.random() * 720) + (360 / ROULETTE_SEGMENTS.length) * resultIndex
  };
  addLog(user.username || "player", `Рулетка: ${segment.label} при ставке ${bet.toFixed(2)} ${CRYPTOBOT_ASSET}`, payout > 0 ? "win" : "loss");
  await persistState();
  await sendJson(res, 200, { ok: true, result: state.lastRoulette, data: bootstrapPayload() });
}
...
async function handleMinesStart(body, res) {
  ...
  if (!Number.isInteger(mineCount) || mineCount < 3 || mineCount > 12) {
    await sendJson(res, 400, { error: "Количество мин: от 3 до 12" });
    return;
  }
  if (!spendBalance(bet)) {
  ...
  addLog(user.username || "player", `Mines стартовал: ставка ${bet.toFixed(2)} ${CRYPTOBOT_ASSET}`, "mines");
  await persistState();
  await sendJson(res, 200, { ok: true, data: bootstrapPayload() });
}
...
async function handleMinesReveal(body, res) {
  ...
  if (!session || !session.active) {
    await sendJson(res, 400, { error: "Сначала начни игру Mines" });
    return;
  }
  ...
  if (session.mines.includes(index)) {
    ...
    addLog(user.username || "player", `Mines: взрыв на клетке ${index + 1}`, "loss");
    await persistState();
    await sendJson(res, 200, { ok: true, data: bootstrapPayload() });
    return;
  }
...
async function handleMinesCashout(res) {
  ...
  if (!session || !session.active || session.safePicks < 1) {
    await sendJson(res, 400, { error: "Нечего выводить из Mines" });
    return;
  }
  ...
  addLog(user.username || "player", `Mines: кэшаут ${payout.toFixed(2)} ${CRYPTOBOT_ASSET}`, "win");
  await persistState();
  await sendJson(res, 200, { ok: true, data: bootstrapPayload() });
}
