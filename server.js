...
function makeGames() {
  return [
    ...
    { id: "crash", name: "Ракета", icon: "↗", note: "в разработке", theme: "locked", live: false },
...
async function handleRouletteSpin(body, res) {
  ...
  const validChoice = ROULETTE_SEGMENTS.find((item) => item.label.toUpperCase() === choice);
  if (!validChoice) {
    awardBalance(bet);
    await sendJson(res, 400, { error: "Выбери доступный множитель" });
    return;
  }
  ...
  const won = segment.label.toUpperCase() === choice;
  const payout = won ? Number((bet * segment.multiplier).toFixed(2)) : 0;
  ...
  addLog(user.username || "player", `X50: ставка на ${validChoice.label}, выпало ${segment.label}, сумма ${bet.toFixed(2)} ${CRYPTOBOT_ASSET}`, payout > 0 ? "win" : "loss");
  await persistState();
