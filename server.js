...
const ADMIN_TELEGRAM_IDS = new Set(
  envStr("BOR_ADMIN_TELEGRAM_IDS", typeof process.env.BOR_DEFAULT_ADMIN_ID === "string" ? process.env.BOR_DEFAULT_ADMIN_ID : "")
    .split(",")
...
async function handleWithdrawal(body, res) {
  ...
  if (autoAllowed) {
    try {
      ...
      withdrawal.auto_error = String(error.message || error).slice(0, 300);
      withdrawal.updated_at = nowIso();
...
async function handleAdminWithdrawalStatus(withdrawalId, body, res) {
  ...
  if (newStatus === "approved") {
    try {
      ...
      withdrawal.updated_at = nowIso();
      await persistState();
      await sendJson(res, 502, { error: `CryptoBot не выполнил перевод: ${withdrawal.auto_error}` });
      return;
    }
  }
