...
function renderX50(roulette) {
  ...
  if (countdownElement) countdownElement.textContent = round.active ? countdown.toFixed(1) : bonus.active ? "BOR" : "10.0";
  ...
  if (spinButtonText) spinButtonText.textContent = bonus.active ? "Выбрать карту" : round.active && bets.length === 1 ? "Поставить ещё" : "Сделать ставку";
...
function renderWithdrawals(withdrawals) {
  ...
  $("#withdrawalList").innerHTML = items.length ? items.map((item) => `
    <div class="admin-list-item">
      <div><strong>@${escapeHtml(item.username)} · ${asMoney(item.amount)} USDT</strong><span>${escapeHtml(translateStatus(item.status))} · ${escapeHtml(translateStatus(item.risk_score))}${item.auto_error ? ` · ${escapeHtml(item.auto_error)}` : ""}</span></div>
