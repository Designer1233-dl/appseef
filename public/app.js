...
function renderX50(roulette, feed) {
  ...
  $("#multiplierRow").innerHTML = playableSegments.map((segment, index) => {
    ...
    return `<button class="multiplier-card ${active ? "selected" : ""} ${placed ? "placed" : ""}" data-x50-choice="${escapeHtml(segment.label)}" type="button"><div class="${x50Class(segment.label)}">${escapeHtml(segment.label)}</div><small>${placed ? `В раунде · ${asMoney(placedBet.amount)} USDT` : `${escapeHtml(player)} · ${escapeHtml(amount)}`}</small></button>`;
  }).join("");
