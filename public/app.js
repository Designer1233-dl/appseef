...
function renderX50(roulette) {
  ...
  $("#rouletteCountdown").textContent = round.active ? countdown.toFixed(1) : bonus.active ? "BOR" : "10.0";
  $(".round-state span").textContent = bonus.active
    ? "выбери одну карту"
    : round.active
      ? `ставок ${bets.length}/${round.maxBets || 2} · деньги списаны`
      : "до следующей игры";

  const canPlace = !bonus.active && (!round.active || bets.length < Number(round.maxBets || 2));
  $("#spinButton").disabled = !canPlace;
  $("#spinButton span").textContent = bonus.active ? "Выбрать карту" : round.active && bets.length === 1 ? "Поставить ещё" : "Сделать ставку";
...
function animateRouletteResult(result) {
  ...
  const finalOffset = Math.round($("#rouletteWindow").clientWidth / 2 - targetCenter);
  track.style.transform = `translate3d(${finalOffset}px,0,0)`;
  track.classList.add("rolling");
  window.setTimeout(() => track.classList.remove("rolling"), 3600);
...
function renderCrash(crash, user) {
  const session = crash || {};
  const active = Boolean(session.active);
  const multiplier = Number(session.multiplier || 1);
  $("#crashMultiplier").textContent = `×${multiplier.toFixed(2)}`;
  $("#crashStatus").textContent = active ? "Ракета летит" : session.status === "crashed" ? `Остановилась на ×${multiplier.toFixed(2)}` : session.status === "cashed_out" ? `Выигрыш забран на ×${multiplier.toFixed(2)}` : "Готова к запуску";
  $("#crashRocket").style.transform = active ? `translate(${Math.min(110, (multiplier - 1) * 22)}px, ${Math.min(70, (multiplier - 1) * -10)}px) rotate(-18deg)` : "";
  $("#startCrash").disabled = active;
  $("#startCrash").textContent = active ? "Ракета летит" : "Запустить ракету";
  $("#cashoutCrash").disabled = !active;
  const outcome = session.lastOutcome;
  $("#crashResult").textContent = outcome?.type === "cashout" ? `Выигрыш ${asMoney(outcome.payout)} USDT зачислен на баланс.` : outcome?.type === "loss" ? `Ракета остановилась на ×${multiplier.toFixed(2)}. Ставка проиграна.` : active ? `Можно забрать: ${asMoney(Number(session.bet || 0) * multiplier)} USDT.` : "Запусти ракету и забери выигрыш до остановки.";
  $("#crashResult").classList.toggle("win", outcome?.type === "cashout");
}
...
function render(data) {
  ...
  renderCrash(data.crash || {}, data.user || {});
...
function bindEvents() {
  ...
  $("#startCrash").addEventListener("click", () => startCrash().catch((error) => { $("#crashResult").textContent = error.message; toast(error.message); }));
  $("#cashoutCrash").addEventListener("click", () => cashoutCrash().catch((error) => { $("#crashResult").textContent = error.message; toast(error.message); }));
