...
async function refresh() {
  try {
    ...
    if (error.message.includes("TELEGRAM_AUTH_REQUIRED") || error.message.includes("Открой BOR внутри Telegram")) {
      document.body.classList.add("auth-required");
      toast("Открой BOR через кнопку Telegram-бота, чтобы увидеть свой профиль");
    }
    throw error;
  }
}
